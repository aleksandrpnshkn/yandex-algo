 ...
 * Количество слов в документе ограниченно.
 * Поэтому индексация зависит от количества документов.
 * Т.е. сложность индексации O(d).
 *
 ...
 * Т.к. релевантными могут быть все документы, то сложность будет O(log d).
 * Итого, в худшем случае O(k * d).
 *
 * Количество ключевых слов на запрос ограничено, поэтому итоговая сложность всех запросов O(q * d).
 *
 * Итоговая сложность программы: O(q * d)
 *
 *
 ...
 * Два крайних случая:
 *      1. Слова в текстах одинаковые. Тогда количество слов очень небольшое, т.к. док содержит <= 1000 символов.
 *          Т.е. память O(d)
 *      2. Во всех текстах слова уникальные. В wordsDocs будет множество слов, у всех один релевантный док.
 *          Т.е. память O(d)
 * Итого O(d).
 *
 * В wordsCountInDocs тоже самое, поэтому O(d).
 *
 ...
 *
 * Итоговая сложность программы: O(d)
 */
